...
int contatecnologia(FILE *arquivo){
    //funcao que conta a quantidade de tecnologias
    int conta;
    char ler;
    char *p;
    int MaxLinha=256;
    fgets(p, MaxLinha, arquivo);
    printf("%s",p);
    char *str1;
    int num1,num2,num3;
    char *str2;
    ...
    int *vet;
    vet=(char*)calloc(50,sizeof(char));
    while (fgets(p, MaxLinha, arquivo) != NULL) {
        if (sscanf(p,"%49[^,],%d,%d,%49[^,],%d", str1, &num1, &num2, str2, &num3) == 5) {
            realloc(str1,strlen(str1));
            realloc(str2,strlen(str2));
            //if()
            //fazer busca binaria nele
            //se não existir ante inserir no final e passar inserction
        }}

    //fread
    //leitura squncial sem repetir
}
    void corrigecabecalho(FILE *arquivo){
    //TODO ESCREVER FUNCAO QUE CORRIGE O CABECALHO COM AS INFORMACOES CORRETAS DE PROXRRN, NRO TECNOLOGIAS E NRO DE PARES DIFERENTES ANTES DE DAR FREE NOS REGISTROS, UTILIZAR CONTADORES
...
void createTable(char *arqCSV,char *arqBIN){
    ...
    //corrigecabecalho(arquivoinicial);
    free(RegDados);
    free(RegCab);
    free(p);
    fclose(arqConvertido);
